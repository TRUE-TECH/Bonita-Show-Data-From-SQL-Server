...
= Product information REST API Extension
REST API to manage Products

The project **productInformationRestAPIExtension** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/productInformationRestAPIExtension.git
cd productInformationRestAPIExtension/
mvn package
----

The build should produce a zip archive under the `target/` folder named `productInformationRestAPIExtension-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `productInformationRestAPIExtension-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
